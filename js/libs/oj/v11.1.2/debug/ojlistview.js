  ...
  const _ojListView = _ListViewUtils.clazz(Object, /** @lends oj._ojListView.prototype */ {
    ...
    HandleFocus: function (event) {
      ...
      if (this.m_active == null) {
        ...
        } else if (item && item.length > 0 && !this.SkipFocus(item) &&
          !this.IsElementEditableOrClickable(target) &&
          !event.target.classList.contains('oj-component')) {
          // focus programmatically set on element within item (including JAWS, see JET-46814)
          // but don't do it if the target is editable/clickable/component so that we do not steal focus
